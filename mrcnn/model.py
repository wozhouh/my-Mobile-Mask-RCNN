...
def resnet_graph(input_image, architecture, stage5=True, train_bn=True):
    ...
    C2 = x = identity_block(x, 3, [64, 64, 256], stage=2, block='c', train_bn=train_bn)  # output: N x 256 x 1/4 x 1/4
    ...
    C3 = x = identity_block(x, 3, [128, 128, 512], stage=3, block='d',
                            train_bn=train_bn)  # output: N x 512 x 1/8 x 1/8
    ...
    C4 = x  # output: N x 1024 x 1/16 x 1/16
    ...
    if stage5:
        ...
        C5 = x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c',
                                train_bn=train_bn)  # output: N x 2048 x 1/32 x 1/32
    else:
        C5 = None
...
def mobilenetv1_graph(input_image, architecture, alpha=1.0, depth_multiplier=1, stage5=True, train_bn=False):
    """MobileNetv1
...
# Taken from the original tf repo. It ensures that all layers have a channel number that is divisible by 8
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, train_bn):
    ...
    # Expand
    x = KL.Conv2D(expansion * in_channels, kernel_size=1, padding='same', use_bias=False, activation=None,
                  name='mobl%d_conv_%d_expand' % (block_id, block_id))(inputs)
    x = BatchNorm(epsilon=1e-3, momentum=0.999, name='bn%d_conv_bn_expand' % block_id)(x, training=train_bn)
    x = KL.Activation(relu6, name='conv_%d_relu' % block_id)(x)

    # Depthwise
    x = KL.DepthwiseConv2D(kernel_size=3, strides=stride, activation=None, use_bias=False, padding='same',
                           name='mobl%d_conv_%d_depthwise' % (block_id, block_id))(x)
    x = BatchNorm(epsilon=1e-3, momentum=0.999, name='bn%d_conv_depthwise' % block_id)(x, training=train_bn)
    x = KL.Activation(relu6, name='conv_dw_%d_relu' % block_id)(x)

    # Project
    x = KL.Conv2D(pointwise_filters, kernel_size=1, padding='same', use_bias=False, activation=None,
                  name='mobl%d_conv_%d_project' % (block_id, block_id))(x)
    x = BatchNorm(epsilon=1e-3, momentum=0.999, name='bn%d_conv_bn_project' % block_id)(x, training=train_bn)
...
def _first_inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, train_bn):
    in_channels = inputs._keras_shape[-1]
    prefix = 'features.' + str(block_id) + '.conv.'
    pointwise_conv_filters = int(filters * alpha)
    pointwise_filters = _make_divisible(pointwise_conv_filters, 8)
    ...
    x = KL.Conv2D(pointwise_filters, kernel_size=1, padding='same', use_bias=False, activation=None,
                  name='mobl%d_conv_%d_project' % (block_id, block_id))(x)
    x = BatchNorm(epsilon=1e-3, momentum=0.999, name='bn%d_conv_project' % block_id)(x, training=train_bn)
...
def mobilenetv2_graph(input_image, architecture, alpha=1.0, stage5=True, train_bn=False):
    ...
    # Stage 1
    x = KL.Conv2D(first_block_filters, kernel_size=3, strides=(2, 2), padding='same', use_bias=False,
                  name='Conv1')(input_image)
    x = BatchNorm(epsilon=1e-3, momentum=0.999, name='bn_Conv1')(x, training=train_bn)
    x = KL.Activation(relu6, name='Conv1_relu')(x)

    C1 = x = _first_inverted_res_block(x, filters=16, alpha=alpha, stride=1, expansion=1, block_id=0, train_bn=train_bn)
    # output: N x 16 x 1/2 x 1/2 (alpha=1.0)

    x = _inverted_res_block(x, filters=24, alpha=alpha, stride=2, expansion=6, block_id=1, train_bn=train_bn)
    C2 = x = _inverted_res_block(x, filters=24, alpha=alpha, stride=1, expansion=6, block_id=2, train_bn=train_bn)
    # output: N x 24 x 1/4 x 1/4 (alpha=1.0)

    x = _inverted_res_block(x, filters=32, alpha=alpha, stride=2, expansion=6, block_id=3, train_bn=train_bn)
    x = _inverted_res_block(x, filters=32, alpha=alpha, stride=1, expansion=6, block_id=4, train_bn=train_bn)
    C3 = x = _inverted_res_block(x, filters=32, alpha=alpha, stride=1, expansion=6, block_id=5, train_bn=train_bn)
    # output: N x 32 x 1/8 x 1/8 (alpha=1.0)

    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=2, expansion=6, block_id=6, train_bn=train_bn)
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1, expansion=6, block_id=7, train_bn=train_bn)
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1, expansion=6, block_id=8, train_bn=train_bn)
    x = _inverted_res_block(x, filters=64, alpha=alpha, stride=1, expansion=6, block_id=9, train_bn=train_bn)
    x = _inverted_res_block(x, filters=96, alpha=alpha, stride=1, expansion=6, block_id=10, train_bn=train_bn)
    x = _inverted_res_block(x, filters=96, alpha=alpha, stride=1, expansion=6, block_id=11, train_bn=train_bn)
    C4 = x = _inverted_res_block(x, filters=96, alpha=alpha, stride=1, expansion=6, block_id=12, train_bn=train_bn)
    # output: N x 96 x 1/16 x 1/16 (alpha=1.0)

    if stage5:
        x = _inverted_res_block(x, filters=160, alpha=alpha, stride=2, expansion=6, block_id=13, train_bn=train_bn)
        x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1, expansion=6, block_id=14, train_bn=train_bn)
        x = _inverted_res_block(x, filters=160, alpha=alpha, stride=1, expansion=6, block_id=15, train_bn=train_bn)
        C5 = x = _inverted_res_block(x, filters=320, alpha=alpha, stride=1, expansion=6, block_id=16, train_bn=train_bn)
        # output: N x 320 x 1/32 x 1/32 (alpha=1.0)
    else:
        C5 = None
...
class PyramidPSROIAlign(KE.Layer):
    ...
    - boxes: [batch, num_boxes, (y1, x1, y2, x2)] in normalized
             ...
             boxes to fill the array.
    - image_meta: [batch, (meta data)] Image details. See compose_image_meta()
    - psroi_score_maps: List of position-sensitive score maps from different levels of the pyramid
                        following the large-separable convolution.
                        Each is [batch, height, width, ps_channels]
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            ...
            # iterate over height
            for k in range(self.pool_shape[0]):
                # iterate over width
                for l in range(self.pool_shape[1]):
                    # find out the area in the feature map matching the bin
                    bin_idx = k * self.pool_shape[0] + l
                    bin_y1 = level_y1 + k * level_h_bin
                    bin_x1 = level_x1 + l * level_w_bin
                    bin_y2 = level_y1 + (k + 1) * level_h_bin
                    bin_x2 = level_x1 + (l + 1) * level_w_bin
                    bin_boxes = tf.concat([bin_y1, bin_x1, bin_y2, bin_x2], axis=1)  # [num_boxes_level, 4]
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, train_bn=False,
                         fc_layers_size=1024):
...
def light_head_classifier_graph(rois, feature_maps, image_meta,
    ...
    x = KL.TimeDistributed(KL.Conv2D(fc_layers_size, (pool_size, pool_size), padding="valid"),
                           name="light_head_class_conv")(x)
    x = KL.TimeDistributed(BatchNorm(), name="light_head_class_bn")(x, training=train_bn)
    x = KL.Activation('relu')(x)
...
# wozhouh: replace the original convolution with the depth-wise and point-wise pair following the Xception-like backbone
def build_mobile_mask_graph(rois, feature_maps, image_meta,
                            pool_size, num_classes, train_bn=True):
    x = PyramidROIAlign([pool_size, pool_size], name="roi_align_mask")([rois, image_meta] + feature_maps)
...
class MaskRCNN:
    ...
    def build(self, mode, config):
        ...
        # wozhouh: add backbone options of MobileNet-v1/MobileNet-v2/Xception
        if callable(config.BACKBONE):
        ...
        if mode == "training":
            ...
            if config.DETECTION_HEAD in ["light-head"]:
            ...
            # the Mask Head
            if config.MASK_HEAD in ["mobile"]:
                mrcnn_mask = build_mobile_mask_graph(rois, mrcnn_feature_maps,
                                                     input_image_meta,
            ...
            # Mask Head
            if config.MASK_HEAD in ["mobile"]:
                mrcnn_mask = build_mobile_mask_graph(detection_boxes, mrcnn_feature_maps,
                                                     input_image_meta,
                                                     config.MASK_POOL_SIZE,
                                                     config.NUM_CLASSES,
                                                     train_bn=config.TRAIN_BN)
    ...
    def get_imagenet_weights(self):
        ...
        # TODO: pre-trained model url for MobileNet-v2/Xception needed further
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        if self.config.BACKBONE in ["resnet50", "resnet101"]:
            ...
            stage_regex = {
                ...
                "2+": r"(conv.*13.*)|(conv.*12.*)|(conv.*11.*)|(conv.*10.*)|"
                      r"(conv.*9.*)|(conv.*8.*)|(conv.*7.*)|(conv.*6.*)|"
                      r"(conv.*5.*)|(conv.*4.*)|(conv.*3.*)|(conv.*2.*)|"
            ...
            stage_regex = {
                ...
                "3+": r"(mobl13.*)|(mobl14.*)|(mobl15.*)|(mobl16.*)|"
                      r"(mobl9.*)|(mobl10.*)|(mobl11.*)|(mobl12.*)|"
                      r"(mobl6.*)|(mobl7.*)|(mobl8.*)|"
                      r"(mobl3.*)|(mobl4.*)|(mobl5.*)|"
                      r"(bn13.*)|(bn14.*)|(bn15.*)|(bn16.*)|"
                      r"(bn9.*)|(bn10.*)|(bn11.*)|(bn12.*)|"
                      r"(bn6.*)|(bn7.*)|(bn8.*)|"
                      r"(bn3.*)|(bn4.*)|(bn5.*)|"
                      r"(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)",
                "4+": r"(mobl13.*)|(mobl14.*)|(mobl15.*)|(mobl16.*)|"
                      r"(mobl9.*)|(mobl10.*)|(mobl11.*)|(mobl12.*)|"
                      r"(mobl6.*)|(mobl7.*)|(mobl8.*)|"
                      r"(bn13.*)|(bn14.*)|(bn15.*)|(bn16.*)|"
                      r"(bn9.*)|(bn10.*)|(bn11.*)|(bn12.*)|"
                      r"(bn6.*)|(bn7.*)|(bn8.*)|"
                      r"(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)",
                "5+": r"(mobl13.*)|(mobl14.*)|(mobl15.*)|(mobl16.*)|"
                      r"(bn13.*)|(bn14.*)|(bn15.*)|(bn16.*)|"
                      r"(mrcnn\_.*)|(rpn\_.*)|(fpn\_.*)"
            }
        ...
        self.keras_model.fit_generator(
            ...
            max_queue_size=self.config.BATCH_SIZE*10,
            workers=workers,
...
def mold_image(images, config):
    ...
    # TensorFlow implementation requires the pixels of the input image scaled between [-1, 1]
